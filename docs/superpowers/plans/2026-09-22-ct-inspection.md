...
- [ ] Environment and Git: initialize a milestone branch, add privacy ignores,
...
- [ ] Tests first: add `tests/test_volume.py`, `tests/test_cases.py` and
...
- [ ] Implement `src/mirai_ct/volume.py`: `load_volume(path)`, `metadata(image)`,
...
- [ ] Implement `cases.py`, `visualization.py`, `__main__.py`: validated local
...
- [ ] Add `notebooks/01_inspect_ct.ipynb`, `cases.example.csv`, README and official
  dataset instructions. Execute notebook through nbclient using ignored outputs.
- [ ] Inspect one public CT and matching label if a verified single-case object
...
- [ ] Review privacy, geometry and memory handling. Run tests, lint, pip check,
...
authorize these routine design choices and commits. Remote selection is pending.
